...
<body>
    ...
    <aside class="admin-sidebar" id="sidebar">
        ...
        <div class="sidebar-section">
            <div class="sidebar-label">{{ app()->getLocale() === 'ar' ? 'القائمة الرئيسية' : 'Main Menu' }}</div>
        ...
        <div class="sidebar-section">
            <div class="sidebar-label">{{ app()->getLocale() === 'ar' ? 'إدارة' : 'Management' }}</div>
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.users.*') ? 'active' : '' }}"
                        href="{{ route('admin.users.index') }}">
                        <i class="bi bi-people-fill"></i> {{ app()->getLocale() === 'ar' ? 'المستخدمون' : 'Users' }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.teams.*') ? 'active' : '' }}"
                        href="{{ route('admin.teams.index') }}">
                        <i class="bi bi-shield-fill"></i> {{ app()->getLocale() === 'ar' ? 'الفرق' : 'Teams' }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.players.*') ? 'active' : '' }}"
                        href="{{ route('admin.players.index') }}">
                        <i class="bi bi-person-badge-fill"></i> {{ app()->getLocale() === 'ar' ? 'اللاعبون' : 'Players' }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.matches.*') ? 'active' : '' }}"
                        ...
                        {{ app()->getLocale() === 'ar' ? 'المباريات' : 'Matches' }}
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="sidebar-section">
            <div class="sidebar-label">{{ app()->getLocale() === 'ar' ? 'البطولات' : 'Competitions' }}</div>
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.competitions.*') ? 'active' : '' }}"
                        href="{{ route('admin.competitions.index') }}">
                        <i class="bi bi-trophy-fill"></i>
                        {{ app()->getLocale() === 'ar' ? 'البطولات' : 'Competitions' }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.types.*') ? 'active' : '' }}"
                        href="{{ route('admin.types.index') }}">
                        <i class="bi bi-tags-fill"></i> {{ app()->getLocale() === 'ar' ? 'الأنواع' : 'Types' }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.subtypes.*') ? 'active' : '' }}"
                        href="{{ route('admin.subtypes.index') }}">
                        <i class="bi bi-bookmark-fill"></i>
                        {{ app()->getLocale() === 'ar' ? 'التصنيفات' : 'Subtypes' }}
                    </a>
                </li>
            </ul>
        </div>
            ...
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.positions.*') ? 'active' : '' }}"
                        ...
                        <i class="bi bi-geo-alt-fill"></i> {{ app()->getLocale() === 'ar' ? 'المراكز' : 'Positions' }}
                    </a>
                </li>
            </ul>
        </div>
            ...
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('home') }}">
                        ...
                        {{ app()->getLocale() === 'ar' ? 'العودة للموقع' : 'Back to Site' }}
                    </a>
                </li>
                <li class="nav-item">
    ...
    <div class="admin-main">
        <div class="admin-topbar">
            <div class="d-flex align-items-center gap-3">
                ...
                <h5 class="mb-0 fw-bold" style="color:var(--dark);font-size:1.1rem;">{{ $title ?? 'لوحة التحكم' }}
                </h5>
            </div>
            <div class="d-flex align-items-center gap-3">
        ...
        <div class="p-4 admin-content">
            @if (session('success'))
                <div class="alert alert-success alert-dismissible fade show d-flex align-items-center gap-2 animate-fade-in-down"
                    role="alert">
            ...
            @endif
            @if (session('error'))
                <div class="alert alert-danger alert-dismissible fade show d-flex align-items-center gap-2 animate-fade-in-down"
                    role="alert">
